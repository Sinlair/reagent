...
import type { ResearchDirectionReport, ResearchDirectionReportStore } from "./researchDirectionReport.js";
...
export class ResearchDirectionReportService {
  ...
  async generate(input: {
    ...
    const paperReports = (await this.paperAnalysisService.listRecent(40)).filter((report) => {
      ...
      return `${report.paper.title} ${report.problemStatement} ${report.coreMethod}`.toLowerCase().includes(topic.toLowerCase());
    });
    ...
    const repoReports = (await this.repoAnalysisService.listRecent(30)).filter((report) => {
      ...
      if (Number.isFinite(createdAtMs) && createdAtMs < cutoffMs) return false;
      if (direction?.id) {
        return `${report.owner}/${report.repo} ${report.description ?? ""}`.toLowerCase().includes(direction.label.toLowerCase());
      }
    ...
    const commonBaselines = unique([
      ...baseline.baselines.map((item) => item.title),
      ...paperReports.flatMap((report) => report.likelyBaselines),
    ]).slice(0, 8);
    ...
    const openProblems = unique([
      ...
      ...paperReports.flatMap((report) =>
        report.conclusions
          .filter((conclusion) => Boolean(conclusion.missingEvidence))
          .map((conclusion) => conclusion.missingEvidence ?? "")
      ),
    ]).slice(0, 8);

    const suggestedRoutes = unique([
      ...baseline.innovationSuggestions,
      ...(direction?.currentGoals ?? []).map((goal) => `Prioritize: ${goal}`),
      ...paperReports.flatMap((report) => report.innovationPoints.map((point) => `Paper cue: ${point}`)),
      ...commonModules.slice(0, 4).map((modulePath) => `Check whether module "${modulePath}" can be adapted for ${topic}.`),
    ]).slice(0, 10);

    const supportingSignals = unique([
      ...baseline.supportingSignals,
      ...discoveryRuns.flatMap((run) => run.directionLabels),
      ...paperReports.map((report) => report.paper.title),
      ...repoReports.map((report) => `${report.owner}/${report.repo}`),
    ]).slice(0, 12);
    ...
    const report: ResearchDirectionReport = {
      ...
      overview: buildOverview({
        ...
        baselineCount: commonBaselines.length,
