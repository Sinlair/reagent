...
function inferStylePenaltyKinds(haystack: string): Array<"too-theoretical" | "too-engineering-heavy"> {
  ...
  if (/\b(theorem|proof|bound|formal|analysis|lemma|proposition)\b/iu.test(haystack)) penalties.push("too-theoretical");
  if (/\b(system|implementation|practical|deployment|benchmark|ablation|module|pipeline)\b/iu.test(haystack)) penalties.push("too-engineering-heavy");
  return penalties;
}
...
export class ResearchFeedbackService {
  ...
  async record(input: {
    ...
    await this.writeStore({ ...store, items: [record, ...store.items].slice(0, MAX_ITEMS) });
    return record;
  }

  async listRecent(limit = 20): Promise<ResearchFeedbackRecord[]> {
    return (await this.readStore()).items.slice(0, Math.max(1, Math.min(limit, 100)));
  }

  async getSummary(limit = 20): Promise<ResearchFeedbackSummary> {
    const store = await this.readStore();
    const counts = emptyCounts();
    for (const item of store.items) counts[item.feedback] += 1;
    return {
  ...
  async scoreDiscoveryCandidate(input: {
    ...
    for (const item of recent) {
      ...
      const matchTokens = uniqueTrimmed([item.topic ?? "", item.paperTitle ?? "", item.notes ?? ""].flatMap((value) => tokenize(value))).slice(0, MAX_MATCHABLE_TOKENS);
      ...
      if (matched) score += appliedWeight;
    }

    return { score, reasons: uniqueTrimmed(reasons) };
  }
}
