...
import type { BaselineSuggestionReport } from "./researchAnalysis.js";
...
export class ResearchBaselineService {
  ...
  async suggest(input: {
  ...
  }): Promise<BaselineSuggestionReport> {
    ...
    const relevantRunTitles = recentDiscoveryRuns
      .filter((run) => {
        ...
        return `${run.directionLabels.join(" ")} ${run.topTitle ?? ""}`
          .toLowerCase()
    ...
    const relevantPaperReports = recentPaperReports.filter((report) => {
      ...
      return `${report.paper.title} ${report.problemStatement}`
        .toLowerCase()
        .includes(topic.toLowerCase());
    });
    ...
    const innovationSuggestions = unique([
      ...
      ...relevantPaperReports.flatMap((report) =>
        report.innovationPoints.map((point) => `Paper cue: ${point}`)
      ),
      ...reusableModules
        .slice(0, 4)
        .map((modulePath) => `Check whether module "${modulePath}" can be adapted for ${topic}.`),
    ]).slice(0, 10);
    ...
    const supportingSignals = unique([
      ...
      ...recentPaperReports.flatMap((report) =>
        report.paper.venue ? [report.paper.venue] : []
      ),
      ...relevantRepoReports.flatMap((report) =>
        report.stars != null
          ? [`${report.owner}/${report.repo} stars=${report.stars}`]
          : [`${report.owner}/${report.repo}`]
      ),
    ]).slice(0, 12);
