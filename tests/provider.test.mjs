...
async function main() {
    ...
    await withTempDir(async (dir) => {
      ...
      try {
        ...
        const inbound = {
          ...
          item_list: [{ type: 1, text_item: { text: "浣犲ソ" } }]
        };
  ...
  await runTest("ChatService plainReply uses direct text generation without tool specs", async () => {
    await withTempDir(async (dir) => {
      const memory = new MemoryService(dir);
      await memory.ensureWorkspace();
      ...
      const reply = await chat.plainReply({
        senderId: "agent-user-plain-1",
        text: "hello",
        source: "wechat"
      });

      assert.equal(reply, "你好！很高兴见到你。");
      assert.equal(requests.length, 1);
      assert.equal("tools" in requests[0], false);
      assert.equal(typeof requests[0].input, "string");
      assert.ok(String(requests[0].input).includes("Current user (agent-user-plain-1): hello"));
    });
  });
