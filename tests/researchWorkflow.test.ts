...
import { rankPapers } from "../src/workflows/paperRanker.js";
import { critiqueResearch } from "../src/workflows/researchCritique.js";
...
async function main(): Promise<void> {
  ...
  await runTest("ResearchWorkflow persists ranked papers, chunks, and critique metadata", async () => {
    ...
    class StubSearchProvider implements PaperSearchProvider {
      async search(): Promise<PaperCandidate[]> {
        return [
          {
            id: "paper-older",
    ...
    class StubContentProvider implements PaperContentProvider {
      async collect(): Promise<{ chunks: ResearchChunk[]; warnings: string[] }> {
        return {
          chunks: [
            {
    ...
    class StubRepository implements ResearchRepository {
      savedReport: ResearchReport | null = null;

      async save(report: ResearchReport): Promise<void> {
        this.savedReport = report;
      }

      async findByTaskId(): Promise<ResearchReport | null> {
